    ...
    return FutureBuilder<List<dynamic>>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return const Center(
              child: Text('Meetings could not load at the moment.'));
        }
        final meetings = snapshot.data ?? const <dynamic>[];
        final proposed =
            meetings.where((item) => _bucket(item) == 'Handoff pending').length;
        final booked =
            meetings.where((item) => _bucket(item) == 'Booked').length;
        final completed =
            meetings.where((item) => _bucket(item) == 'Completed').length;
        final missed =
            meetings.where((item) => _bucket(item) == 'Missed').length;
          ...
          child: Padding(
            ...
            child:
              ...
              _MeetingPanel(
                  ...
                  emptyLabel:
                      'Handoff, booked, completed, and missed meetings appear here.',
                  items: meetings
                      .map((item) => _MeetingRow.fromRaw(item))
                      .toList()),
            ]),
          ),
        );
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
              'This page now reflects real client meeting records derived from client-visible reply and handoff activity.',
              style: Theme.of(context)
                  .textTheme
                  .bodyLarge
                  ?.copyWith(color: AppTheme.publicMuted))
...
class _MeetingPanel extends StatelessWidget {
  const _MeetingPanel(
      {required this.title, required this.emptyLabel, required this.items});
  final String title;
...
class _MeetingRow {
  ...
  factory _MeetingRow.fromRaw(dynamic raw) {
    ...
    final primary =
        _join([_bucket(map), _scheduledLabel(_read(map, 'scheduledAt'))]);
    final secondary = _join([
      _firstNonEmpty([_read(lead, 'companyName'), _read(lead, 'fullName')]),
      _read(campaign, 'name'),
      _read(map, 'bookingUrl')
    ]);
    return _MeetingRow(title: title, primary: primary, secondary: secondary);
  }
  final String title;
...
String _bucket(dynamic raw) {
  ...
  if (status.contains('booked') || status.contains('confirm')) return 'Booked';
...
String _join(List<String> values) =>
    values.where((e) => e.trim().isNotEmpty).join(' · ');
