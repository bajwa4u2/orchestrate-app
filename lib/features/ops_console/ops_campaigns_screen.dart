...
class OpsCampaignsScreen extends StatefulWidget {
  ...
  @override
  State<OpsCampaignsScreen> createState() => _OpsCampaignsScreenState();
}

class _OpsCampaignsScreenState extends State<OpsCampaignsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final data = await _repo.fetchCampaignLifecycle();
      if (!mounted) return;
      final raw = data['campaigns'] ?? data['rows'] ?? data['data'];
      setState(() {
        _campaigns = raw is List
  ...
  Future<void> _loadHistory(String id) async {
    ...
    try {
      final data = await _repo.fetchCampaignAudit(id, limit: 5);
      if (!mounted) return;
  ...
  Future<void> _runAction(String id, String endpoint, String label,
      {bool confirm = false, bool needsReason = false}) async {
    String? reason;
    if (confirm || needsReason) {
      final r = await _reasonDialog(context, label, needsReason: needsReason);
      if (r == null) return;
      reason = r;
    }
    setState(() => _action[id] = _Act.working(label));
    try {
      await _repo.rawPost(endpoint, body: reason != null && reason.isNotEmpty ? {'reason': reason} : {});
      if (!mounted) return;
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return _CampaignCard(
          ...
          onAction: (ep, label, {bool confirm = false, bool needsReason = false}) =>
              _runAction(id, ep, label, confirm: confirm, needsReason: needsReason),
        );
      },
    );
  }
...
class _CampaignCard extends StatelessWidget {
  const _CampaignCard({
    required this.campaign, required this.expanded, required this.action,
  ...
  final void Function(String ep, String label, {bool confirm, bool needsReason}) onAction;

  String get _id => campaign['id'] as String? ?? '';
  String get _name => campaign['name'] as String? ?? _id;
  String? get _clientName => campaign['clientName'] as String?;
  String? get _status => campaign['status'] as String? ?? campaign['lifecycleState'] as String? ?? campaign['state'] as String?;
  bool get _hasHold => campaign['hasContinuityHold'] as bool? ?? false;

  Color _statusColor(String? s) {
    switch (s?.toUpperCase()) {
      case 'ACTIVE': return AppTheme.emerald;
      case 'PAUSED': return AppTheme.amber;
      case 'DRAFT': return AppTheme.subdued;
      case 'COMPLETED': return AppTheme.accent;
      case 'ARCHIVED': return AppTheme.subdued;
      default: return AppTheme.subdued;
    }
  }

  Color get _severity {
    if (_hasHold) return AppTheme.amber;
    final s = _status?.toUpperCase();
    if (s == 'PAUSED') return AppTheme.amber;
    if (s == 'ACTIVE') return AppTheme.emerald;
    return AppTheme.subdued;
  }

  List<Map<String, dynamic>> get _evidence => [
    {'label': 'Status', 'value': _status ?? '—'},
    if (_hasHold) {'label': 'Continuity hold', 'value': 'Yes', 'severity': 'warning'},
    if (campaign['activeSinceDays'] != null)
      {'label': 'Active since', 'value': '${campaign['activeSinceDays']}d'},
    if (campaign['totalLeads'] != null)
      {'label': 'Total leads', 'value': campaign['totalLeads']},
    if (campaign['readyLeads'] != null)
      {'label': 'Ready leads', 'value': campaign['readyLeads']},
    if (campaign['sentCount'] != null)
      {'label': 'Sends (total)', 'value': campaign['sentCount']},
  ];

  String get _diagnosis {
    final s = _status?.toUpperCase();
    if (_hasHold) return 'Campaign has an active continuity hold. The execution engine has paused dispatch. Clear the hold to re-evaluate and resume.';
    switch (s) {
      case 'ACTIVE': return 'Campaign is active. Monitor lead inventory and dispatch rate for anomalies.';
      case 'PAUSED': return 'Campaign is paused — no dispatch until resumed. Confirm whether this is intentional.';
      case 'DRAFT': return 'Campaign is in draft. Activate to begin the send cycle.';
      case 'COMPLETED': return 'Campaign completed. No further dispatch. Review for archival.';
      case 'ARCHIVED': return 'Campaign archived. Read-only.';
      default: return 'State unrecognized. Check campaign configuration.';
    }
  }
          ...
          child: Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (_clientName != null)
                  Text(_clientName!, style: const TextStyle(fontSize: 11, color: AppTheme.subdued)),
              ])),
              if (_hasHold)
                Container(margin: const EdgeInsets.only(right: 6),
                    padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                    decoration: BoxDecoration(color: AppTheme.amber.withOpacity(0.1), borderRadius: BorderRadius.circular(3)),
                    child: const Text('HOLD', style: TextStyle(fontSize: 9, color: AppTheme.amber, fontWeight: FontWeight.w700))),
              if (_status != null)
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (action?.isWorking ?? false)
                ...
                Wrap(spacing: 8, runSpacing: 6, children: [
                  if (isDraft)
                    _OutcomeBtn(label: 'Activate', outcome: 'RESOLVE',
                        onTap: () => onAction('/campaigns/$_id/activate', 'Activate')),
                  if (isActive)
                    _OutcomeBtn(label: 'Pause', outcome: 'INTERRUPT',
                        onTap: () => onAction('/operator/campaigns/$_id/pause', 'Pause', confirm: true, needsReason: true)),
                  if (isPaused)
                    _OutcomeBtn(label: 'Resume', outcome: 'RESOLVE',
                        onTap: () => onAction('/operator/campaigns/$_id/resume', 'Resume', needsReason: true)),
                  if (_hasHold)
                    _OutcomeBtn(label: 'Clear hold', outcome: 'RESOLVE',
                        onTap: () => onAction('/execution/campaigns/$_id/clear-continuity-hold', 'Clear hold', confirm: true)),
                  _OutcomeBtn(label: 'Force discovery', outcome: 'RESOLVE',
                      onTap: () => onAction('/signals/discovery/profiles/$_id/run-now', 'Force discovery')),
                  _OutcomeBtn(label: 'Force adaptation', outcome: 'RESOLVE',
                      onTap: () => onAction('/adaptation/campaigns/$_id/run', 'Force adaptation')),
                ]),
              ...
              const SizedBox(height: 6),
              _VerifSource('campaigns/$_id/audit', 'rows[0].action', null),
              const SizedBox(height: 14),
...
// ─── Shared widgets (duplicated pattern, private to this file) ─────────────────
...
class _ScreenHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    ...
    if (onRefresh != null)
      OutlinedButton.icon(onPressed: onRefresh, icon: const Icon(Icons.refresh, size: 16),
...
class _RingLabel extends StatelessWidget {
  const _RingLabel(this.l);
  final String l;
  @override
  Widget build(BuildContext context) => Text(l.toUpperCase(),
...
class _EvidTable extends StatelessWidget {
  ...
  static Color _sc(String? s) {
    switch (s) { case 'critical': return AppTheme.rose; case 'warning': return AppTheme.amber; default: return AppTheme.muted; }
  }
  @override
  Widget build(BuildContext context) {
...
class _HistList extends StatelessWidget {
  ...
  static String _ts(String iso) {
    try { final dt = DateTime.parse(iso).toLocal(); return '${dt.year}-${_p(dt.month)}-${_p(dt.day)} ${_p(dt.hour)}:${_p(dt.minute)}'; } catch (_) { return iso; }
  }
...
Future<String?> _reasonDialog(BuildContext context, String label, {bool needsReason = false}) async {
  ...
  return showDialog<String>(
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: needsReason ? TextField(controller: ctrl,
        style: const TextStyle(fontSize: 13, color: AppTheme.text), maxLines: 2,
        decoration: InputDecoration(hintText: 'Reason', hintStyle: const TextStyle(color: AppTheme.subdued, fontSize: 12),
            ...
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(AppTheme.radius), borderSide: BorderSide(color: AppTheme.accent.withOpacity(0.6))))) : const Text('Confirm this action?', style: TextStyle(fontSize: 13, color: AppTheme.muted)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel', style: TextStyle(color: AppTheme.subdued))),
        FilledButton(onPressed: () => Navigator.pop(ctx, ctrl.text.trim().isEmpty ? '' : ctrl.text.trim()),
