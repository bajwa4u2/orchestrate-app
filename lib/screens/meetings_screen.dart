...
class _MeetingRow { const _MeetingRow({required this.title, required this.primary, required this.secondary}); factory _MeetingRow.fromRaw(dynamic raw){ final map=_asMap(raw); final lead=_asMap(map['lead']); final campaign=_asMap(map['campaign']); final client=_asMap(map['client']); final title=_firstNonEmpty([_read(map,'title'), _read(lead,'companyName'), _read(lead,'fullName'), _read(client,'displayName'), 'Meeting']); final primary=_join([_bucket(map), _scheduledLabel(_read(map,'scheduledAt'))]); final secondary=_join([_firstNonEmpty([_read(lead,'companyName'), _read(lead,'fullName'), _read(client,'displayName')]), _read(campaign,'name'), _read(map,'bookingUrl')]); return _MeetingRow(title:title, primary:primary, secondary:secondary);} final String title; final String primary; final String secondary; }
